 ...
 */
public class FlexibleTreeNode2<T extends IGuifiable> extends DefaultMutableTreeNode implements IDebugable {
	 ...
	 */
	public FlexibleTreeNode2(T userObj) {
		super(userObj);
		...
		// but it's quite ok.
		ErrorHandler.Assert(userObj != null, true, FlexibleTreeNode2.class, "No user object provided!");
	}
...
//	}
	...
	@SuppressWarnings("unchecked")
	private void toDebugString_recursive(StringBuilder sb, int indentLvl, FlexibleTreeNode2<T> tn) {
		toDebugString_indent(sb, indentLvl);
		sb.append(tn.toString());
		sb.append("\n");
		for (TreeNode tnChild : tn.getChildren()) {
			toDebugString_recursive(sb, indentLvl + 1, (FlexibleTreeNode2<T>) tnChild);
		}
	}
	...
	public String toString() {
		try {
			...
			ErrorHandler.logException(th, false, FlexibleTreeNode2.class, "Error resolving toGuiString()");
	 ...
	 */
	public Iterable<FlexibleTreeNode2<T>> getChildren() {
		@SuppressWarnings("rawtypes")
		final Enumeration enumChildren = this.children();
		return new Iterable<FlexibleTreeNode2<T>>() {
			public Iterator<FlexibleTreeNode2<T>> iterator() {
				return new Iterator<FlexibleTreeNode2<T>>() {
					public boolean hasNext() {
						return enumChildren.hasMoreElements();
					}

					@SuppressWarnings("unchecked")
					public FlexibleTreeNode2<T> next() {
						return (FlexibleTreeNode2<T>) enumChildren.nextElement();
					}
	 ...
	 */
	public /* static */ List<FlexibleTreeNode2<T>> getAllTreeNodesAsList(FlexibleTreeNode2<T> tnStart) {
		...
		List<FlexibleTreeNode2<T>> lstToReturn = new ArrayList<>(); // Ineffizient!!!!!
		...
		for (int i = 0; i < tnStart.getChildCount(); i++) {
			...
			FlexibleTreeNode2<T> tnChild = (FlexibleTreeNode2<T>) tnStart.getChildAt(i);
			
			List<FlexibleTreeNode2<T>> lstFromChild = getAllTreeNodesAsList(tnChild);
	...
	private void getAllTreeNodesAsListMoreEfficient_recursive(List<FlexibleTreeNode2<T>> lstResults,
			FlexibleTreeNode2<T> tnCurrent) {

		lstResults.add(tnCurrent);

		for (int i = 0; i < tnCurrent.getChildCount(); i++) {
			@SuppressWarnings("unchecked")
			FlexibleTreeNode2<T> tnChild = (FlexibleTreeNode2<T>) tnCurrent.getChildAt(i);
	 ...
	 * @return list with the tree nodes.
	 */
	public List<FlexibleTreeNode2<T>> getAllTreeNodesAsListMoreEfficient() {

		List<FlexibleTreeNode2<T>> lstResults = new ArrayList<>(countTreeNodes(this) + 5);
	 ...
	 * @return list with the tree nodes.
	 */
	public /* static */ List<FlexibleTreeNode2<T>> getAllSubTreeNodesAsList(FlexibleTreeNode2<T> tnStart) {
		
		List<FlexibleTreeNode2<T>> lstRet = new ArrayList<>();
		...
		for (int i = 0; i < tnStart.getChildCount(); i++) {
			...
			List<FlexibleTreeNode2<T>> lstCh = getAllSubTreeNodesAsList(tnChild);
			lstRet.addAll(lstCh);
		}
	...
	private static <Z extends IGuifiable> void getAllTreeNodesAsListMoreEfficient_recursive(FlexibleTreeNode2<Z> tnStart,
			List<FlexibleTreeNode2<Z>> lstToReturn) {
		lstToReturn.add(tnStart);
		for (int i = 0; i < tnStart.getChildCount(); i++) {
			@SuppressWarnings("unchecked")
			FlexibleTreeNode2<Z> tnChild = (FlexibleTreeNode2<Z>) tnStart.getChildAt(i);
	 ...
	 * @return list with the tree nodes.
	 */
	public static <Z extends IGuifiable> List<FlexibleTreeNode2<Z>> getAllTreeNodesAsListMoreEfficient_old(
			FlexibleTreeNode2<Z> tnStart) {
		List<FlexibleTreeNode2<Z>> lstToReturn = new ArrayList<FlexibleTreeNode2<Z>>();
		if (tnStart == null) {
			return lstToReturn;
		}
	...
	public List<FlexibleTreeNode2<T>> getAllTreeNodesAsListMoreEfficient_old() {
	 ...
	 */
	public List<FlexibleTreeNode2<T>> getAllNodesWithCondition(Predicate<FlexibleTreeNode2<T>> cond) {
